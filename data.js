...
function renderAluno(id, tab){
  ...
  ({resumo:tabResumo,dieta:tabDieta,dia:tabDia,treino:tabTreino,checkin:tabCheckin,evolucao:tabEvolucao,medidas:tabMedidas,dados:tabDados}[tab]||tabResumo)(a, body);
}
...
function tabTreino(a, body){
  ...
  const treinos=t.treinos.map(tr=>`<div class="block"><h4>${esc(tr.nome)} <span class="pill">${esc(tr.foco)}</span></h4>
  ...
  body.innerHTML=`
    <div class="card"><div class="spread"><h2 style="margin:0">Plano de treino</h2><span class="pill info">${t.dias}x/sem · ${esc(t.experiencia)}</span></div>
      <p class="muted" style="margin:.3rem 0 0">Intensidade: ${esc(t.rir)} · ${esc(t.repScheme)}</p></div>
    ${treinos}
    <div class="alert"><b>Cadência de execução</b>${esc(t.cadencia)}</div>
    <div class="card tight"><h3>Volume semanal por grupo</h3><table>${volRows}</table>
      <p class="field-help">Faixa alvo: 10–20 séries por grupo por semana. Progrida a carga quando bater o topo das reps com técnica.</p></div>
    <div class="alert"><b>Cardio</b>${esc(t.cardio)}</div>
    <div class="alert warn"><b>Deload</b>${esc(t.deload)}</div>
    <div class="alert"><b>Descanso entre séries</b>Compostos pesados pedem mais descanso (força e segurança); isoladores, menos. Os tempos já vêm ajustados ao objetivo na tabela de cada treino.</div>`;
}
